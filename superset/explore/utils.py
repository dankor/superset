...
def check_dataset_access(dataset_id: str) -> Optional[bool]:
    if dataset_id:
        # Access checks below, no need to validate them twice as they can be expensive.
        dataset = DatasetDAO.find_by_id_or_uuid(dataset_id, skip_base_filter=True)
        if dataset:
...
def check_query_access(query_id: str) -> Optional[bool]:
    if query_id:
        # Access checks below, no need to validate them twice as they can be expensive.
        query = QueryDAO.find_by_id_or_uuid(query_id, skip_base_filter=True)
        if query:
...
def check_datasource_access(
    datasource_id: str, datasource_type: DatasourceType
) -> Optional[bool]:
    if datasource_id:
        try:
...
def check_access(
    ...
    if not chart_id:
        return True
    # Access checks below, no need to validate them twice as they can be expensive.
    chart = ChartDAO.find_by_id_or_uuid(chart_id, skip_base_filter=True)
    if chart:
